...
-- /docker-entrypoint-initdb.d) and referenced by the k8s configmap init job.
...
CREATE TABLE IF NOT EXISTS job_postings (
    ...
    mongo_jd_ref  TEXT,             -- ObjectId of the raw JD doc in MongoDB
...
CREATE TABLE IF NOT EXISTS status_events (
    ...
    source         TEXT NOT NULL DEFAULT 'manual' CHECK (source IN ('manual','email_parsed')),
    occurred_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_status_events_app ON status_events(application_id);

-- Valid pipeline statuses (documented, enforced at the app layer):
